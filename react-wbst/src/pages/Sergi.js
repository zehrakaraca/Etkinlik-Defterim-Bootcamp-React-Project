...
function Sergi() {
    const { Tur } = useParams();
    const [sergiEvents, setSergiEvents] = useState([]);
    const [searchTerm, setSearchTerm] = useState("");
  
    useEffect(() => {
      const filteredEvents = Events.filter((event) => event.Tur === "SERGİ");
      setSergiEvents(filteredEvents);
    }, [Tur]);
